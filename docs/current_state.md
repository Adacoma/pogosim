...
- The MSYS2/MinGW-w64 CI has been exercised on hosted Windows. The first PowerShell/MSVC run reached Box2D configuration but could not resolve the hard-coded Visual Studio 2022 generator; the revised developer-environment/Ninja path has not yet been observed there, and local validation cannot reproduce MSVC.
...
- The Windows jobs now distinguish MSYS2/UCRT64 with MinGW-w64 from a native PowerShell/MSVC build. Both use the same pinned Box2D 3.x revision; the MSYS2 job retains the simulator flash-state smoke test and native-CPython suite, while the MSVC job initializes the latest installed `cl.exe`, builds through Ninja, and validates the CMake library artifact.
